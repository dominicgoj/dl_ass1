    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for data, target in test_data:
                ...
                loss = test_loss_fn(prob, target).item()
...
def main():
    ...
    X_train_tensor, y_train_tensor = torch.from_numpy(transformed_X_train_split).float(), torch.from_numpy(y_train).reshape(-1, 1).float()
    X_test_tensor, y_test_tensor = torch.from_numpy(transformed_X_test_split).float(), torch.from_numpy(y_test).reshape(-1, 1).float()
    ...
    y_hat_numpy_prediction = (y_hat_numpy > 0.5).astype(float)
    plot_confusion_matrix(y_true=y_test_numpy, y_pred=y_hat_numpy_prediction,
